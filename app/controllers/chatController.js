...
async function chatWithBoth(req, res) {
  try {
    ...
    await chatService.addMessageToChat(chatId, "assistant", chatGPTResponse); //deepseek and openai apis understand the "assistant" role as the answers generated by them. Then when loading
    await chatService.addMessageToChat(chatId, "assistant", deepSeekResponse); //the history in the getOpenAIResponse/getDeepseekResponse they will know how to "continue" the conversation

    res.json({
      chatId,
      responses: { chatGPT: chatGPTResponse, deepSeek: deepSeekResponse },
    });
  } catch (error) {
    console.error("Error in the controller:", error);
    res.status(500).json({ error: error.message });
